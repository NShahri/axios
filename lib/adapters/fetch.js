...
function reponseParser(config, request, response) {
  if (config.responseType === 'arraybuffer') {
    ...
    //
    // TODO: when responseType is document
    //
...
function fetchAdapter(config) {
  ...
  if (request.headers.get(contentTypeHeaderName)) {
    console.log(contentTypeHeaderName,
      request.headers.get(contentTypeHeaderName));
  ...
  if (config.timeout !== undefined) {
    //
    // TODO: implement timeout to abort request
    //
  ...
  return fetch(request).catch(function(err) {
    ...
    return reponseParser(config, request, response);
  }).then(function(result) {
    return new Promise(function(resolve, reject) {
      // console.log('RESULT', result, result.data, result.request.headers, result.config.headers);
      settle(resolve, reject, result);
    });
  }).catch(function(err) {
    console.log('ERR', err.message, config, config.headers);
    throw err;
  });
}
